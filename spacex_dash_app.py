...
# Create Dataframe from launch data csv
...
def get_pie_chart(entered_site):    
    if entered_site == 'ALL':
        success_pie_chart = px.pie(spacex_df, 
                     names = 'Launch Site', 
                     title = 'Total successful launches by site')        
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        # filtered_df = filtered_df.groupby('class').count().reset_index()        
        filtered_df = filtered_df.groupby('Booster Version Category').count().reset_index()        
        print(filtered_df)
        print(spacex_df)
        success_pie_chart = px.pie(filtered_df, 
                     names = 'Booster Version Category', 
                     title = 'Total launches for site: {}'.format(entered_site))        
        # return the outcomes piechart for a selected site
    return success_pie_chart
...
def get_scatter_chart(entered_site, payload_range):
    if entered_site == 'ALL':
        ...
        success_payload_scatter_chart = px.scatter(filtered_df, 
                        x = 'Payload Mass (kg)', 
    ...
    return success_payload_scatter_chart
